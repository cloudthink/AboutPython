...
from Base import Generator4d
from RNN.RNN import Basic3d
from NNUtil import Activations
...
class CNN(Basic4d):
    ...
    def init_from_data(self, x, y, x_test, y_test, sample_weights, names):
        if self.height is None or self.width is None:
            assert len(x.shape) == 4, "未提供宽和高, 因此 len(x.shape) 应该是 4"
            self.height, self.width = x.shape[1:3]
        if len(x.shape) == 2:
            x = x.reshape(len(x), self.height, self.width, -1)
        else:
            assert self.height == x.shape[1], "高被设置为 {}, but {} found".format(self.height, x.shape[1])
            assert self.width == x.shape[2], "宽被设置为 {}, but {} found".format(self.height, x.shape[2])
    ...
    def _build_model(self, net=None):
        ...
        if net is None:
            net = self._tfx
        for i, (filter_size, n_filter, pooling) in enumerate(zip(self.filter_sizes, self.n_filters, self.poolings)):
