...
def download_data():
    ...
    for idx in IDS:
        ...
        data_file = os.path.join(data_folder, "{}.csv".format(idx))
...
def main():
    ...
    for idx in IDS:
        numerical_idx = np.load("_Data/idx/{}.npy".format(idx))#获取numerical_idx
        local_params = copy.deepcopy(base_params)
        local_params["name"] = str(idx)
        local_params["data_info"]["numerical_idx"] = numerical_idx
        local_params["data_info"]["file_type"] = 'txt'
        #模型创建，参数搜索，k_random搜索参数评估
