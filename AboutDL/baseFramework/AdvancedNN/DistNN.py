...
from NNUtil import *
from BasicNN.DistNN import Basic
...
class Advanced(Basic):
    ...
    def init_model_structure_settings(self):
        super(Advanced, self).init_model_structure_settings()
    ...
    def print_settings(self, only_return=False):
        msg = "\n".join([
            "=" * 100, "当前是 {}".format("{}-分类问题".format(self.n_class) if not self.n_class == 1
                else "回归问题"), "-" * 100,"Data     : {} training samples, {} test samples".format(
                len(self._train_generator), len(self._test_generator) if self._test_generator is not None else 0),
            "Features : {} categorical, {} numerical".format(len(self.categorical_columns), np.sum(self.valid_numerical_idx))]) + "\n"
