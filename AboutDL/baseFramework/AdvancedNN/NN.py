...
class Advanced(Basic):
    ...
    def init_model_param_settings(self):
        super(Advanced, self).init_model_param_settings()#继承并定义再BasicNN中定义过的模型超参数
    ...
    def init_model_structure_settings(self):
        ...
        if not self._use_wide_network:#只在wide模型中使用DNDF,所以不用wide就不用dndf了
            self._dndf = None
        else:
    ...
    #参数i代指当前正在获取第i个离散型特征的Embeddin，参数n代指该离散型特征的特征取值个数
    def _get_embedding(self, i, n):
        embedding_size = math.ceil(math.log2(n)) + 1 if self.embedding_size == "log" else self.embedding_size
        #利用“截断正态分布”来初始化，初始化均值、标准差分别为0，0.02
        embedding = tf.Variable(tf.truncated_normal( [n, embedding_size], mean=0, stddev=0.02 ), name="Embedding{}".format(i))
    ...
    #实现ANN中的线性映射，net线性映射的输入，shape权值矩阵的shape，appendix线性映射这一套运算步骤所属的name_scope的后缀
    def _fully_connected_linear(self, net, shape, appendix):
        with tf.name_scope("Linear{}".format(appendix)):
            ...
            if self._pruner is not None:#如果使用剪枝技术的话，就调用相应方法剪枝
    ...
    def _build_layer(self, i, net):
        if self.use_batch_norm:#模型超参数：是否使用BN规范化
        ...
        if self.dropout_keep_prob < 1:#模型超参数：是否使用Dropout
    ...
    def _get_feed_dict(self, x, y=None, weights=None, is_training=True):
        #获取连续型特征，如果有离散型特征存在的话就利用self.valid_numerical_idx来将所有连续型特征提取出来。最后一位代指标签的类型，所有要把最后一位排除在外
