...
class VisUtil:
    ...
    def get_line_info(weight, max_thickness=4, threshold=0.2):
        ...
        thicknesses = np.array([[int((max_thickness - 1) * abs(n)) + 1 for n in line] for line in weight])
    ...
    def get_graphs_from_logs():
        ...
        for (hus, ep, bt), log in logs.items():
            ...
            title = "hus: {} ep: {} bt: {}".format("- " + " -> ".join(hus) + " -", ep, bt)
            ...
            plt.savefig("Results/img/" + "{}_{}_{}".format("-".join(hus), ep, bt))
            plt.close()
...
class Overview:
    ...
    def run(self, ans, y_pred, images):
        ...
        for i in range(n_row):
            if i == n_row - 1:
                pictures.append(np.hstack([*results[i * n_col:], np.zeros((256, 640 * (n_row * n_col - len(results)), 3)) + 255]).astype(np.uint8))
            else:
                pictures.append(np.hstack(results[i * n_col:(i + 1) * n_col]).astype(np.uint8))
